...
- **Newest local correction (2026-09-25 / DP-0.3):** Meeting output profiles now control provider caps, room total budgets, and unlimited synthesis instructions. Unlimited uses 12K ordinary-turn and 16K synthesis caps; creative/planning prompts require language-matched, actionable expansion. Requesting another round compacts prior Canonical State records into one summary Claim while retaining transcript history. Incomplete provider work preserves the prior memo and exposes interrupted-round recovery. Lint/build pass; offline tests remain 64/65 because the existing Solo session-key test returns 502. No live calls or deployment.

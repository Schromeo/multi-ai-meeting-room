...
- **DP-0.2 local portability pass, remote CI pending (D-066, 2026-09-19):** a repository-owned vinext launcher removes shell-specific environment syntax; source tests normalize line endings; pinned Wrangler generation plus an optional inactive D1 augmentation makes type checking accurate; and `pnpm check` now regenerates worker types, builds, runs 63/63 tests, lints, and type-checks in one command. Frozen install and the full check pass on Windows. A parsed GitHub Actions matrix applies the same contract to Windows and Ubuntu; its first remote run remains pending. DP-0.2 remains Current; DP-0.3 has not started. See [local validation](evaluations/2026-09-19-dp-0-2-local-portability.md).
...
DP-0.0 direction ratification and DP-0.1 repository truth are complete. DP-0.2 Engineering Portability Baseline is the only Current implementation milestone. The exact sub-milestones, dependencies, budgets, acceptance checks, and stop rules live in [Detailed Development Milestones](DEVELOPMENT_MILESTONES.md).

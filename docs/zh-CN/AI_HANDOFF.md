...
- 阶段：M2 真实 Discuss 实现完成，等待真实供应商验收。
- 已完成：OpenAI、Anthropic、Gemini 统一适配；服务端密钥；token 流式输出；独立提案；指定交叉审阅；综合；一次追加修订；人工批准/拒绝；用量估算；停止控制；模拟端到端测试。
- 未完成：生产 API Key、真实模型评测、持久化、证据系统、审计数据模型、执行连接器和比较评测。
- 当前里程碑：M2 真实 Discuss 房间。
- 下一决定：配置至少两个供应商密钥并完成第一次真实评测。

...
M2 已实现真实 Discuss 协议：模型适配、流式输出、有限轮次、交叉审阅、决策 memo、人工决策门、用量估算，以及不保存凭证的浏览器本地会议档案。2026-08-04，真实 OpenAI + Anthropic 会议已经走通端到端路径；在修正模型专用请求参数兼容性并把证据扩展到更多问题前，M2 仍为当前阶段。

...
- **本地产品版本：** v0.6。
- **可运行基线：** 真实流式 Discuss、可复用模型席位、会话 BYOK、人工决定 Gate 和浏览器本地会议历史。
...
- **最近实现 Gate：** 修复 OpenAI 模型专用可选参数、创建命名 v0.6 备份，然后开始 M2.7。
- **已批准下一架构：** 会议协议蓝图 v1，已经写入文档但尚未实现。
- **关键路径：** 真实基线 -> 本地 Event Store -> Canonical Meeting State -> 可恢复的 Chair 编排器 -> Observer 与定向辩论 -> Whiteboard 与 Follow-up -> 比较评测。
...
实现进度：产品和协议功能已完成，并通过模拟端到端流式测试。2026-08-04，真实 OpenAI + Anthropic 房间完成提案、交叉审阅、综合、用量报告、保存和 Human Gate。M2 在修复 OpenAI 模型专用可选参数并扩大比较证据前保持“当前”。
...
实现进度：会议发言、memo、人工决定、用量和席位摘要已经保存到有数量上限的浏览器本地档案。用户可以新建房间而不删除旧记录、重新打开记录，并显式删除记录。凭证不会进入档案。账号归属、服务端同步、持久事件存储、导出和跨设备恢复仍未完成。
...
完成条件：已完成或被中断的房间可以在刷新后通过本地 Event 与最新有效 Snapshot 重建，同时不恢复任何凭证。
